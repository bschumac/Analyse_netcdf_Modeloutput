#### This script will calculate the RMSE of SWDR_300 of the regCM Output compared 
...
lst_fcl <- lapply(fld_lst, function(i){
    ...
    for (i in seq(1, length(plots_names))){
      ...
      print(rownames(act_netcdf_rsns_kili_vals)[1])
      ...
      ME_rsns <- mean( act_merge$rsns - act_merge$rsns_plot, na.rm = TRUE)
      #MAE <- mean(abs(pred_vals - obs_vals), na.rm = TRUE)
      MAE_rsns <- mean(abs( act_merge$rsns - act_merge$rsns_plot), na.rm = TRUE)
      
      png(filename=paste0(filebase_results, fld_o,
                          j, "_rsns.png"), width = 1280, height = 960)
      plot( act_merge$time, act_merge$rsns_plot, lwd = 2,
            type="l", ylim=c(0,1200), main = paste0("Time series of rsns at ", j))
      lines( act_merge$time, act_merge$rsns, col="red", lty=2, lwd = 2)
      legend("bottomright", c("rsns Plot", "rsns Model"), col=c("black", "red"), lty=c(1,2))
      dev.off()
