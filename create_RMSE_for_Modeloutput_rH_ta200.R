#### This script will calculate the RMSE of the regCM Output compared 
...
calcErrorScores <- function (netcdf_tas, netcdf_hurs, out.path){
    ...
    plots_csv <- read.csv("/home/benjamin/Data_masterthesis/plots_csv/plots.csv")
    ...
    plot(ext1, add=T)
    plots_shp <- spTransform(plots_shp, proj4string(netcdf_hurs))
    ...
    all_rmse_vals<- lapply(plots_names, function(i){
      ...
      png(filename=paste0("/home/benjamin/Data_masterthesis/results_error_calc/",
                            i, "_rH.png"), width = 1280, height = 960)
      plot( act_merge$time, act_merge$rH_plot, lwd = 2,
            type="l", ylim=c(0,100), main = paste0("Time series of rH at ", i))
      ...
      dev.off()
      png(filename=paste0("/home/benjamin/Data_masterthesis/results_error_calc/",
                          i, "_ta200.png"), width = 1280, height = 960)
      plot( act_merge$time, act_merge$Ta_200_plot, lwd = 2,
            type="l", ylim=c(270,310), main = paste0("Time series of Ta_200 at ", i))
    ...
    write.csv(error_stats.out.df, file = "/home/benjamin/Data_masterthesis/results_error_calc/error_stats.csv")
